...
const interval = setInterval(() => {
    if (currentCount <= 0) {
        //game.style.backgroundImage = './assets/game-over.jpg';
...
function verifyLookPokemon(to) {
    ...
    const pokemonRightPosition =
        to === "ArrowLeft" || to === "KeyA"
            ? `${getRightPosition() - 64}px`
            : `${getRightPosition() + 64}px`;

    if (findCharmander) {
        const newTopPosition = (to === "ArrowUp" || to === "KeyW"
            ? `${getTopPosition() + 8}px`
            : `${getTopPosition() - 8}px`);
    ...
    switch (event.code) {
        case "ArrowLeft":
            if (getRightPosition() < 770) {
                ash.style.right = `${getRightPosition() + 7}px`;
